...
DROP PROCEDURE IF EXISTS getSchoolNameWithCID;
DELIMITER //
CREATE PROCEDURE getSchoolNameWithCID
(inCID INTEGER)
BEGIN
SELECT 
	S.SNAME
...
#Search for a set of equivalent courses when given
DROP PROCEDURE IF EXISTS searchEquivalent;
DELIMITER //
CREATE PROCEDURE searchEquivalent
(IN inNAME1 VARCHAR(30), inNAME2 VARCHAR(30), inTITLE1 VARCHAR(140), inTITLE2 VARCHAR(140), inDEPARTMENT1 VARCHAR(30), inDEPARTMENT2 VARCHAR(30), inSCHOOLNAME1 VARCHAR(100), inSCHOOLNAME2 VARCHAR(100))
BEGIN
SELECT 
	E.IS_EQUIVALENT, E.EC_COMMENT, E.APPROVED, C1.CID, C1.CNAME, C2.CID, C2.CNAME, C2.TITLE, C2.DEPARTMENT, S2.SNAME, S2.STATE, S2.CITY, S2.ADDRESS
FROM
...
WHERE
	E.COURSE1 = C1.CID
    AND E.COURSE2 = C2.CID
	AND C1.SID = S1.SID
    AND C2.SID = S2.SID
    AND C1.CNAME LIKE inNAME1
    AND C2.CNAME LIKE inNAME2
    AND C1.TITLE LIKE inTITLE1
    AND C2.TITLE LIKE inTITLE2
    AND C1.DEPARTMENT LIKE inDEPARTMENT1
    AND C2.DEPARTMENT LIKE inDEPARTMENT2
	AND S1.SNAME LIKE inSCHOOLNAME1
	AND S2.SNAME LIKE inSCHOOLNAME2;
END //
DELIMITER ;
