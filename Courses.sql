...
BEGIN
SELECT 
    S.SID, S.SNAME, S.STATE, S.CITY, S.ADDRESS
FROM
    SCHOOL AS S
WHERE
    S.SNAME LIKE SNAME;
...
#Get set of every SID with given OID
DROP PROCEDURE IF EXISTS getSIDwithOID;
DELIMITER //
CREATE PROCEDURE getSIDwithOID
...
BEGIN
SELECT 
	C.CNAME, C.CID, S.SNAME, S.STATE, S.CITY, S.ADDRESS
FROM
    COURSE AS C,
    SCHOOL AS S
WHERE
